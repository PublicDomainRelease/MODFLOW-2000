...
C2------ READ MXACTS, NSS, NTRIB, ISTCB1, AND ISTCB2.
    ...
    4 FORMAT(1X,'MAXIMUM OF',I5,' ACTIVE STREAM NODES AT ONE TIME',/
     1   1X,'NUMBER OF STREAM SEGMENTS IS ',I5,/
     2   1X,'NUMBER OF STREAM TRIBUTARIES IS ',I5)
...
C9------PRINT AMOUNT OF SPACE USED BY STREAM PACKAGE.
      ISP=ISPA+ISPD
      WRITE (IOUT,8)ISP
    8 FORMAT(1X,I8,' ELEMENTS IN RX ARRAY ARE USED FOR STREAMS')
      ISP=ISPB+ISPC+ISPE+ISPF
      WRITE (IOUT,9)ISP
    9 FORMAT(1X,I8,' ELEMENTS IN IR ARRAY ARE USED FOR STREAMS')
C
C10-----RETURN.
      RETURN
      END
...
C3A-----IF THERE ARE NEW NON-PARAMETER STREAM CELLS, READ THEM
   ...
   99          FORMAT(1X,/1X,'THE NUMBER OF ACTIVE STREAM CELLS (',
     1              I4,') IS GREATER THAN MXACTS(',I4,')')
               STOP
            END IF
...
C3------PRINT NUMBER OF REACHES IN CURRENT STRESS PERIOD.
  ...
  101 FORMAT(1X,/1X,I5,' STREAM REACHES')
C
...
C9-----READ AND PRINT TRIBUTARY SEGMENTS.
   ...
   10 FORMAT(/,30X,'MAXIMUM NUMBER OF TRIBUTARY STREAMS IS ',I5,//1X,
