...
C  DEFINE (DF) PROCEDURE
     ...
     &                LCAMPA,LCAMCA,LCAAP)
      CALL GWF1SFR1DF(NLAKES,NLAKESAR,LKACC7,LCSTAG,LSLAKE,LSTGLD,
     ...
     &                LSFLOB,LSRTCO,LSCLKO,LSALKI,LSALKO,NSSAR)
...
C4------ALLOCATE SPACE IN RX AND IR ARRAYS.
     ...
     &                       NPHFB,MXHFB,IHFB,NOPRHB)
        IF(IUNIT(44).GT.0) THEN
          CALL GWF1SFR1ALP(ISUMRX,ISUMIR,LCSTRM,ICSTRM,NSTRM,
     &        IUNIT(44),IOUT,ISTCB1,ISTCB2,NSS,CONST,MAXPTS,DLEAK,
     &        LCSEG,ICSEG,LCOTSG,LCXSEC,LCIVAR,LCQSTG,IUNIT(22),
     &        ISTRIN,ISTROT,LCOTFLW,LCDVFLW,IUNIT(15),NSOL,
     &        LSCOUT,LSCONQ,LSCNRN,LSCNTB,LSSLIN,LSCQIN,LSCGW,ISTGLD,
     &        IDSTRT,LSSIN,LSSOUT,LSCOTO,LKACC7,LCSTAG,LSLAKE,ISTGNW,
     &        LSCPPT,LCNSEG,NSTRPAR,NSEGDIM,LCSFRQ,NSTRMAR,NSSAR)
        ENDIF
CLAK
        IF(IUNIT(22).GT.0)
     ...
     *     LKCUM6,LKCUM7,LKCUM8,LKCUM9,NSSAR,NLAKESAR)
CLAK
        IF(IUNIT(46).GT.0)
...
C         Allocate array for MNW1 site IDs
          IF (ITERPK.EQ.1) THEN
            ALLOCATE (MNWSITE(MXWEL2),STAT=ISTAT)
            IF (ISTAT.NE.0) THEN
              WRITE(*,703)ISTAT
...
C---------FLOW-SIMULATION OPTIONS
     ...
     &                   NFHBX2,IFHBD3,IFHBD5,NHEDDIM,NFLWDIM)
        IF(IUNIT(44).GT.0)
     1      CALL GWF1SFR1RPP(RX(LCSTRM),IR(ICSTRM),
     ...
     5       RX(LSCNRN),RX(LSCPPT),NSOL,IOUTS,NSTRPAR,NSEGDIM,ITERPK,
     &       INAMLOC,IG(LCIBOU),NCOL,NROW,NLAY)
        IF(IUNIT(18).GT.0)
...
C----------READ USING PACKAGE READ AND PREPARE MODULES.
     ...
     &                          NNPCHD,NPCHD,IPCBEG,NOPRCH)
          IF(IUNIT(44).GT.0)
     1        CALL GWF1SFR1RPS(RX(LCSTRM),IR(ICSTRM),KKPER,NSTRM,
     2            IUNIT(44),IOUT,RX(LCSEG),IR(ICSEG),IR(LCNSEG),NSS,
     3            IR(LCIVAR),IR(LCOTSG),MAXPTS,
     4            IPTFLG,RX(LCXSEC),RX(LCQSTG),IUNIT(15),RX(LSCONQ),
     5            RX(LSCNRN),RX(LSCPPT),NSOL,IOUTS,NSTRPAR,NSEGDIM)
          IF (IUNIT(44).GT.0 .AND. ISEN.GT.0)
     &        CALL GWF1SFR1SEN(IOUTG,NPLIST,NSEGDIM,IR(ICSEG),
     &                           IX(LCISEN))
CLAK
     ...
     *        RX(LKCLKI),RX(LKCUM1),RX(LKCUM2),RX(LKCUM3),RX(LKCUM4),
     ...
     3             LKNODE,X(LCVKA),GX(LCBOTM),NBOTM)
            ...
            END IF
            IF (IUNIT(44).GT.0)
     &          CALL GWF1LAK3SFR1RPS(NTRB,NDV,NLAKES,IR(INTRB),IR(INDV),
     &                  NSS,NSSAR,IR(LCIVAR),IR(LCOTSG),IOUT,NODES,
     &                  GX(LCBUFF))
          END IF
CLAK
...
CLAK
     ...
     6                            ISSFLG(KKPER),IUNIT(9))
              IF (IUNIT(44).GT.0)
     1               CALL GWF1SFR1FM(RX(LCSTRM),IR(ICSTRM),
     2        GZ(LCHNEW),GX(LCHCOF),GX(LCRHS),IG(LCIBOU),NSTRM,NCOL,
     3        NROW,NLAY,IOUT,NSS,RX(LCSEG),IR(ICSEG),IR(LCOTSG),
...
C--STREAM-AQUIFER RELATIONS (SFR1 PACKAGE)
C-----ADDED DELEAK MAY 12, 2004
            IF (IUNIT(44).GT.0)
     1               CALL GWF1SFR1BD(RX(LCSTRM),IR(ICSTRM),GZ(LCHNEW),
     ...
     &       RX(LCSFRQ),NSEGDIM,IUNIT(22),DLEAK)
C--LAKES
            IF (IUNIT(22).GT.0)
