C     Last change:  ERB  26 Jan 2001    1:46 pm
...
C   ----------------------------------------------------------------
     ...
     4    +IAT+LSL1+LSL2+2*LSL3+LSL4+4*LSL5+9*LSL6+3*LSL7
     5    +ISMN+2*ISMX
      ISPIR=ISPB+ISPD+ISPE+ICN+ICO+IMT+IMT1+ICS+ICDR+ISS1+IAC12I+LSL4
C
...
C
     ...
     5 CAUG,CPPT,CRNF,NSOL,IOUTS,SSMN,SSMX,ISS)
C
...
C
  ...
  149 FORMAT(/1X,'MAXIMUM NUMBER OF GRID CELLS ADJACENT TO LAKES HAS BEE
     1N EXCEEDED WITH CELL ',3I5,'  REDIMENSION VARIABLES ISPA, ISPA1, I
     2SPB IN MODULE LAK3AL')
      STOP
  147 ILAKE(1,M) = K
      ILAKE(2,M) = J
      ILAKE(3,M) = I
...
C
     ...
     1        NROW,NCOL,NLAY,FLOB,IUNITGWT,LKNODE) 
C
...
C       SEEPAGE INTO OR OUT OF A LAKE WALL NODE, WHEN ILAKE(5,L)=1 OR 2.
...
C
     ...
     9      IUNITGAGE,NUMGAGE,IGGLST,VOLOLD,FLOB,IGENLK,ISS,LAYHDT)
C
...
C     ------------------------------------------------------------------
     ...
     5 VBVL(4,MSUM),VBNM(4,MSUM),BUFF(NCOL,NROW,NLAY),STAGES(NLAKES),
     ...
     9 IGGLST(3,NUMGAGE),VOLOLD(NLAKES)
      DIMENSION FLOB(LKNODE),IGENLK(NCOL,NROW,NLAY)
     ...
     3 TEXT(4),LDRY(NODES),BOTTMS(NLAKES),SUMCNN(NLAKES),SUMCHN(NLAKES)
      DIMENSION ILB(5),IRB(5),ICB(5)
      DATA TEXT(1),TEXT(2),TEXT(3),TEXT(4) /'   L','AKE ',' SEE','PAGE'/
C     ------------------------------------------------------------------
C
...
C
C3A-----TEST TO SEE IF CELL-BY-CELL TERMS ARE NEEDED.
      IF((ICBCFL.NE.0).AND.(ILKCB.GT.0)) IBD=1
C
C3B-----CLEAR CELL-BY-CELL BUFFER.
      DO 5 IL=1,NLAY
      DO 5 IR=1,NROW
      DO 5 IC=1,NCOL
         IF (IUNITGWT.GT.0) IGENLK(IC,IR,IL)=0
...
C10------COMPUTE ACCUMULATIVE LAKE NODAL AREA, EXCLUDE WALL NODES.
          ...
          IF (ILKCB.LT.0.AND.ICBCFL.NE.0) WRITE(IOUT,880) (TEXT(N),
     1          N=1,4),KPER,KSTP,L,IL,IR,IC,RATE
880       FORMAT(1X,4A4,'   PERIOD',I3,'   STEP',I3,'   NODE',I4,
...
885         RATOUT=RATOUT-RATE
            ...
            IF (IUNITGWT.GT.0) IGENLK(IC,IR,IL)=1
            GO TO 900
C
...
C
 ...
 1086 IF(LWRT.GT.0.OR.ICBCFL.LE.0) GO TO 1041
C
...
C
 ...
 1025 FORMAT(/1X,'PERIOD ',I5,5X,'TIME STEP ',I5,5X,'TIME STEP LENGTH ',
     ...
     2   E11.4)
            WRITE(IOUT,1040)
 1040 FORMAT(//5X,'HYDROLOGIC BUDGET SUMMARIES FOR SIMULATED LAKES'
     1    ,/,5X,'-----------------------------------------------')    
            WRITE(IOUT,1045)
 1045 FORMAT(1X,'LAKE',3X,'STAGE',5X,'PRECIP',7X,'EVAP',7X,'RUNOFF')
       ...
       IF(RNF(NN).LT.ZERO) RUNF =-RNF(NN)*PRCPLK(NN)*BGAREA(NN)
         RUNFD = RUNF*DELT
 1100 WRITE(IOUT,1050) NN,STGNEW(NN),PPTIN,EOUT,RUNFD
 ...
 1046 FORMAT(/12X,'GROUND WATER',12X,'SURFACE WATER'/1X,'LAKE',4X,
     1 'INFLOW',5X,'OUTFLOW',6X,'INFLOW',5X,'OUTFLOW')
            DO 1101 NN=1,NLAKES
               ...
               QSIN=SURFIN(NN)
               QSOUT=SURFOT(NN)
 1101 WRITE(IOUT,1051) NN,QIN,QOUT,QSIN,QSOUT
 ...
 1105 WRITE(IOUT,1055) NN,WDRAW,FLXINL(NN),VOL(NN),SURFA(NN),
     2           DELH(NN),TDELH(NN)
...
C     THICKNESS FOR ALKES WHEN BCF PACKAGE IS USED
C     ******************************************************************
C
C     ------------------------------------------------------------------
C     SPECIFICATIONS:
...
C
      WRITE(IOUT,108)
  108 FORMAT(//9X,'C',15X,'INTERFACE CONDUCTANCES FOR LAKES'/
     7  3X,'L',5X,'O'/3X,'A',5X,'L',2X,'L',2X,'T',
     1  6X,'(IF TYPE = 1 OR 2, CONDUCTANCES ARE PER UNIT THICKNESS.)'/
     6  3X,'Y',2X,'R',2X,'U',2X,'A',2X,'Y'/
     5  3X,'E',2X,'O',2X,'M',2X,'K',2X,'P',
     2  24X,'LAKEBED',6X,'C O N D U C T A N C E S'/3X,'R',2X,'W',2X,
     *  'N',2X,'E',
     3  2X,'E',5X,'DELTA Y',3X,'DELTA X',2X,'LEAKANCE',3X,'LAKEBED',3X,
     4  'AQUIFER',2X,'COMBINED'/1X,79('_'))
C
      DO 350 II=1,LKNODE
      K = ILAKE(1,II)
      J = ILAKE(2,II)
