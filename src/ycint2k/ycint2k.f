C     Last change:  ERB   9 Jul 2001    3:57 pm
      PROGRAM YCINT2K
C
...
C     NPD MUST EQUAL OR EXCEED THE NUMBER OF PARAMETER VALUES
      ...
      PARAMETER (VERSION='1.0 07/20/2000')
     ...
     &        NVAR, J, MPR, N, NDD, NDMH, NDMHD, NH, NOBS, NPD
      PARAMETER (NDD=2000,NPD=100,NDMHD=1000)
      CHARACTER*12 DID(NDD), DID1(NDD)
      DIMENSION X(NPD,NDD), C(NPD,NPD), H(NDD), H1(NDD), X1(NPD,NDD),
     &          V(NDD), V1(NDD), WQ(NDMHD), STATS(4), ISYM(NDD),
     &          ISYM1(NDD)
