C     Last change:  ERB   9 Jul 2001    3:55 pm
      PROGRAM BEALE2K
C
...
C-------ASSIGN VERSION NUMBER AND DATE
      ...
      PARAMETER (VERSION='1.1.1 05/07/2001')
...
Cerb change VAR from single to double precision
     ...
     &        NRES, NVAR
      PARAMETER (NDD=10000,NHD=10000,NDMHD=1000,NPD=500,MPD=100,IPD=100)
      CHARACTER*10 PARNAM(NPD)
      CHARACTER*12 OBSNAM(NDD)
      DIMENSION BOPT(NPD), FOPT(NDD), B(NPD), FC(NDD), FL(NDD), 
...
C     CHECK ARRAY DIMENSIONS
      IERR = 0
      IF (NVAR.GT.NPD) THEN
        WRITE(IOUT,1410) 'NUMBER OF PARAMETERS',NPD,'NPD'
        IERR = 1
      ENDIF
      IF (NH.GT.NHD) THEN
        WRITE(IOUT,1410) 'NUMBER OF HEAD OBSERVATIONS',NHD,'NHD'
        IERR = 1
      ENDIF
      IF (NDMH.GT.NDMHD) THEN
        WRITE(IOUT,1410) 'NUMBER OF OBSERVATIONS OTHER THAN HEADS',
     &                   NDMHD,'NDMHD'
        IERR = 1
      ENDIF
      IF (NOBS.GT.NDD) THEN
        WRITE(IOUT,1410) 'TOTAL NUMBER OF OBSERVATIONS',NDD,'NDD'
        IERR = 1
      ENDIF
      IF (MPR.GT.MPD) THEN
...
C           PRIOR WITH FULL WEIGHT MATRIX
  ...
  160       CONTINUE
            SUMC = SUMC + SUMCC*( B(NIPR(J))-BOPT(NIPR(J)) )
            SUMD = SUMD + SUMDD*( B(NIPR(J))-BOPT(NIPR(J)) )
            SUMY(M) = SUMY(M) + SUMYY*( BPRI(J)-B(NIPR(J)) )
  170     CONTINUE
        ENDIF
