...
C-------RETURN
     ...
     1                RELAX,IPRPCG,IN,IOUT,MUTPCG,NITER)
C
...
C-------READ HCLOSE,RCLOSE,RELAX,NBPOL,IPRPCG,MUTPCG
      READ(IN,1) HCLOSE,RCLOSE,RELAX,NBPOL,IPRPCG,MUTPCG
...
C
     ...
     3      NLAY,NODES,RELAX,IOUT,MUTPCG,IU,IP,SN,SP,SR)
...
C     ------------------------------------------------------------------
      ...
      DOUBLE PRECISION HNEW,HHCOF,RRHS
...
C     ------------------------------------------------------------------
C
C      IF(KITER.EQ.1.AND.KSTP.EQ.3.AND.KPER.EQ.2)
...
C--------CHANGED 510 FORMAT 01SEPT1991 AND 20MAR1992
  510      FORMAT(//,'  CHOLESKY DIAGONAL LESS THAN OR EQUAL TO ZERO',
     1        ' -- EXECUTION TERMINATED',/,' (MATRIX NOT DIAGONALLY ',
     2        'DOMINANT -- IN SOME SITUATIONS SETTING RELAX < 1.0 ',
...
C
     ...
     1                  NITER,MXITER,IOUT,NPCOND,BPOLY)
C
C
...
C
    ...
    5 FORMAT(1H0,'MAXIMUM HEAD CHANGE FOR EACH ITERATION:'/
     1    1H0,4('    HEAD CHANGE  LAYER,ROW,COL')/1X,120('-'))
      WRITE (IOUT,10) (HCHG(J),(LHCH(I,J),I=1,3),J=1,NITER)
      WRITE(IOUT,11)
      WRITE(IOUT,15)
   15 FORMAT(1H0,'MAXIMUM RESIDUAL FOR EACH ITERATION:'/
     1    1H0,4('     RESIDUAL    LAYER,ROW,COL')/1X,120('-'))
      WRITE (IOUT,10) (RCHG(J),(LRCH(I,J),I=1,3),J=1,NITER)
      WRITE(IOUT,11)
C
      RETURN
C
